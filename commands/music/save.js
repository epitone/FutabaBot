			...
			args: [
				{
                    ...
                    type: 'string',
                    validate: playlist_name => { return !playlist_name.includes('-') }
                }
			]
        });
    }

    async run(message, { playlist_name }) {
        ...
        let result = await musicService.SavePlaylist(message.guild, playlist_name, message.member);
        if(!Array.isArray(result) || !result.length) {
            ...
            if(!result.includes(undefined)) {
                discordUtils.embedResponse(message, {
                    color: `ORANGE`,
                    description: `**${message.author.tag}** I successfully saved the songs under the “${playlist_name}” playlist.`
                }); 
            } else {
